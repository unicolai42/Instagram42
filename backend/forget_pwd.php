<?PHP
    ...
    if ($_SERVER['SERVER_NAME'] == 'localhost')
        $message .= '
    http://localhost:8080/backend/change_pwd.php?username='.urlencode($username).'&cle='.urlencode($cle);
    else
        $message .= '
    https://'.$_SERVER['SERVER_NAME'].'/backend/change_pwd.php?username='.urlencode($username).'&cle='.urlencode($cle);
